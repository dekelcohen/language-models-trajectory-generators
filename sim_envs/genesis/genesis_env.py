...
def build_arg_parser():
    ...
    # Tracker fallbacks: same group as main.py (minus --tracking), because
    # ``run_simulation_environment`` reads them off ``args`` when a START_TRACKING payload
    # omits a field and the launcher does not forward them.
    from tracking.session import add_tracking_args

    add_tracking_args(parser, include_enable=False)
    return parser
