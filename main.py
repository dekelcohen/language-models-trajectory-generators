...
if __name__ == "__main__":
    ...
    # The functions below are in the inpterpreter context of exec(code generated by the VLM). 
    exec_locals = get_exec_locals(api, logger)    
            
    try:
        # NO-LLM Debug: Parse conversations, extract python code blocks and executes them 
        if args.replay_log:
            logger.info(PROGRESS + f"Replay mode activated. Parsing: {args.replay_log}" + ENDC)
            execute_blocks_from_log(args.replay_log, api, logger)
            logger.info(PROGRESS + f"Replay mode finished executing code blocks" + ENDC)
